...
@interface GetYoutooTv (PrivateMethods)

- (void)didStopLoadData;
@end

@implementation GetYoutooTv
...
@synthesize getYoutooTvString;
@synthesize getYoutooTvTextView;
...
- (void)viewWillAppear:(BOOL)animated
{
  ...
  /*  NSString *path = [NSString stringWithFormat:@"http://www.youtoo.com/iphoneyoutoo/showHelp/getyoutoo"];
...
//		[defaults setObject:[NSDate date] forKey:@"BlogPageLastUpdate"];
        ...
        [tempURLStr appendFormat:@"http://www.youtoo.com/iphoneyoutoo/showHelp/getyoutoo"];
...
{
    [super viewDidLoad];
    self.title = @"Get Youtoo Tv";
    activityIndicator = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(138, 180, 40, 40)];    
    [activityIndicator setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleGray];
    [self.view addSubview:activityIndicator]; 
   getYoutooTvTextView = [[UITextView alloc] initWithFrame:CGRectMake(10.0, 95.0, 310.0, 350.0)];
    [getYoutooTvTextView setBackgroundColor:[UIColor clearColor]];
    [getYoutooTvTextView setFont:[UIFont systemFontOfSize:16.0]];
     getYoutooTvTextView.textColor=[UIColor colorWithRed:0.14 green:0.29 blue:0.42 alpha:1];
    [getYoutooTvTextView setText:@""];
    [getYoutooTvTextView setText:getYoutooTvString ];
    //friendTxtFld.tag = indexPath.row;
    [getYoutooTvTextView setEditable:NO];
    [self.view addSubview:getYoutooTvTextView];
    // Do any additional setup after loading the view from its nib.
}
#pragma mark -
...
- (void)didEndParsing
{
    self.isNetworkOperation = NO;
	[activityIndicator stopAnimating];
	[getYoutooTvTextView setText: self.getYoutooTvString];
}
