...
- (void)dealloc
{
	...
	[self cancelLoad];
    [queue release]; // Prabakar: newly added
	[userInfo release];
	[postBody release];
...
{
	[[self cancelledLock] lock];
	//queue = newQueue;   // Prabakar: It is original, commented and added below
    if (newQueue != queue) {   // Prabakar: newly added
        [queue release];   // Prabakar: newly added
        queue = [newQueue retain];  // Prabakar: newly added
    }  // Prabakar: newly added
	[[self cancelledLock] unlock];
}
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (userAgentString) {
            userAgentString = @"youtoo/1.0 CFNetwork/485.13.9 Darwin/11.0.0";
...
+ (NSString *)defaultUserAgentString
{
	...
	// From http://www.cocoawww.com/index.pl?DeterminingOSVersion
...
// From: http://www.cocoawww.com/index.pl?BaseSixtyFour
